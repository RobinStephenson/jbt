...
sealed class RoboticonFactory
{
    ...
    public void BuyRoboticon(Inventory inv)
    {
        ///if (inv.Money > PRICE) {
        inv.AddItem(ItemType.Roboticon, 1);
        ///inv.AddMoney(-PRICE);
        ///}
        ///else {
    ...
    public void BuyCustomisation(Roboticon roboticon, RoboticonCustomisation customisation, Inventory inv)
    {
        if (customisationRequirementsTest(roboticon.CurrentCustomisations, customisation.Prerequisites))
        {

            if (inv.Money > customisation.Price)
            {
                roboticon.CurrentCustomisations.Add(customisation);
                roboticon.SetBonusProduction(customisation.ResourceType, customisation.BonusMultiplier);
                inv.AddItem(ItemType.Roboticon, 1);
